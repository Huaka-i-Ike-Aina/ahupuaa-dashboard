...
plant_biome_data <- read.csv(
  "scraped_data/plant_biome_list.csv",
  stringsAsFactors = FALSE
)

full_plant_data <- read.csv(
  "scraped_data/hawaii_native_plants_complete.csv",
  stringsAsFactors = FALSE
)

full_plant_data$Plant_Name <- gsub(
  "^Native Plants Hawaii - Viewing Plant : ",
  "",
  full_plant_data$Plant_Name
)
full_plant_data$Plant_Name <- gsub(
  "^Native Plants Hawaii - Viewing Plant",
  "",
...
full_plant_data$Plant_Name <- trimws(full_plant_data$Plant_Name)

ahupuaa_carbon_data_full <- st_read(
...
# Function to get plants for specific biomes
get_plants_for_biomes <- function(biome_list, plant_data) {
  # Remove any NA or empty biomes
  biome_list <- biome_list[!is.na(biome_list) & biome_list != ""]

  if (length(biome_list) == 0) {
    return(character(0))
  }

  # Create regex pattern to match any of the biomes
  pattern <- paste(biome_list, collapse = "|")

  # Filter plants that have any of these biomes
  matching_plants <- plant_data %>%
    filter(grepl(pattern, Biomes, fixed = FALSE)) %>%
    pull(Plant_Name) %>%
    unique()
...
observeEvent(input$toggle_carbon, {
  if (!carbon_overlay_visible()) {
    ...
    ahupuaa_carbon_data <- ahupuaa_carbon_data_full %>%
      filter(
        mokupuni == input$mokupuni,
        moku == input$moku,
    ...
    carbon_palette <- colorFactor(
      ...
      domain = unique(ahupuaa_carbon_data$biome_unit) # the text column
    )
    ...
    output$plant_table <- renderDataTable(
      {
        # Get matching plants
        unique_biomes <- unique(ahupuaa_carbon_area_summary$biome_unit)
        all_matching_plants <- get_plants_for_biomes(
          unique_biomes,
          plant_biome_data
        )

        # Rank plants by biome abundance
        ranked_plants <- plant_biome_data %>%
          filter(Plant_Name %in% all_matching_plants) %>%
          filter(!is.na(Biomes) & Biomes != "") %>%
          rowwise() %>%
          mutate(
            # Split biomes for this plant
            plant_biomes = list(strsplit(Biomes, " \\| ")[[1]]),
            # Calculate max area among this plant's biomes
            max_biome_area = {
              areas <- ahupuaa_carbon_area_summary$total_area_acres[
                ahupuaa_carbon_area_summary$biome_unit %in% plant_biomes
              ]
              if (length(areas) > 0) max(areas, na.rm = TRUE) else 0
            },
            # Get the most abundant biome name for this plant
            primary_biome = {
              matching_biomes <- ahupuaa_carbon_area_summary %>%
                filter(biome_unit %in% plant_biomes) %>%
                arrange(desc(total_area_acres))
              if (nrow(matching_biomes) > 0) {
                matching_biomes$biome_unit[1]
              } else {
                NA_character_
              }
            }
          ...
          ungroup() %>%
          # Replace Inf with 0 for plants with no matching biomes
          mutate(
            max_biome_area = ifelse(
              is.infinite(max_biome_area) | is.na(max_biome_area),
              0,
              max_biome_area
            )
          )

        # Function to truncate and add expand functionality
        truncate_text <- function(text, max_chars = 150, id_prefix = "text") {
          ...
          text_id <- paste0(
            id_prefix,
            "_",
            abs(as.integer(charToRaw(paste(text, collapse = ""))[1]))
          )

          # Truncate text
          short_text <- substr(text, 1, max_chars)

          # Create expandable HTML
          sprintf(
            '<span id="%s_short">%s... <a href="#" onclick="document.getElementById(\'%s_short\').style.display=\'none\'; document.getElementById(\'%s_full\').style.display=\'inline\'; return false;" style="color: #0066cc; text-decoration: underline;">Read more</a></span><span id="%s_full" style="display:none;">%s <a href="#" onclick="document.getElementById(\'%s_short\').style.display=\'inline\'; document.getElementById(\'%s_full\').style.display=\'none\'; return false;" style="color: #0066cc; text-decoration: underline;">Read less</a></span>',
            text_id,
            short_text,
            text_id,
            text_id,
        ...
        # Join with full plant data and create display table
        plant_table <- ranked_plants %>%
          left_join(
            full_plant_data,
            by = "Plant_Name",
            suffix = c("_biome", "_full")
          ) %>%
          # Sort by biome area (descending), then by plant name
          arrange(desc(max_biome_area), Plant_Name) %>%
          ...
          mutate(
            ...
            max_biome_area = round(max_biome_area, 1)
          ) %>%
          # Truncate long text fields
          rowwise() %>%
          mutate(
            Early_Hawaiian_Use = truncate_text(
              Early_Hawaiian_Use,
              max_chars = 150,
              id_prefix = paste0("early_use_", row_number())
            ),
            Modern_Use = truncate_text(
              Modern_Use,
              max_chars = 150,
              id_prefix = paste0("modern_use_", row_number())
            )
          ) %>%
          ungroup() %>%
          # Make URLs clickable
          mutate(
            URL = sprintf('<a href="%s" target="_blank">View</a>', URL_biome)
          ) %>%
          select(-URL_biome)
      ...
      options = list(
        ...
        autoWidth = TRUE,
        columnDefs = list(
          list(targets = c(12, 13), width = '300px'),
          list(targets = '_all', className = 'dt-left dt-top')
        )
      ),
      escape = FALSE,
      colnames = c(
    ...
    leafletProxy("main_map") %>%
      addPolygons(
        ...
        group = "Carbon Data",
        fillColor = ~ carbon_palette(biome_unit),
        color = "black",
        weight = 1,
        fillOpacity = 0.7,
        popup = ~ paste0(
